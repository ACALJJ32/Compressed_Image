...
def main():
    ...
    if cuda:
        ...
        logging.info(f"===> GPU id: '{opt.gpus}'")
    ...
    logging.info("Random Seed: ", opt.seed)
    ...
    for epoch in range(start_epoch, opt.nEpochs):
        ...
        if opt.validation:
            psnr_mean = validate(valid_data_loader, model, epoch)
            psnr_list.append(psnr_mean)
            if curr_psnr < psnr_mean:
                curr_psnr = psnr_mean
                max_psnr_epoch = epoch
            
            # save best checkpoint
            save_checkpoint(model, epoch)
            print("curr psnr: {} | max psnr: {}, epoch: {}".format(psnr_mean, max(psnr_list), max_psnr_epoch+1))
            logging.info("curr psnr: {} | max psnr: {}, epoch: {}".format(psnr_mean, max(psnr_list), max_psnr_epoch+1))
        scheduler.step()
...
def train(training_data_loader, optimizer, model, criterion, epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        if iteration % 100 == 0 or iteration == len(training_data_loader):
            ...
            logging.info(f"===> Epoch[{epoch+1}]({iteration}/{len(training_data_loader)}): Loss: {loss.item():.6f}, Time: {usetime:.4f}")

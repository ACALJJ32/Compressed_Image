...
parser.add_argument("--realdatapath", default='inputs/video_val_frames', type=str, help="test data path")
...
parser.add_argument("--save", default="video_val_results", type=str, help="save path")  
...
clip_list = os.listdir(opt.realdatapath)
for clip in clip_list:

    fullPaths = sorted(glob.glob(os.path.join(opt.realdatapath, clip, '*.png')))
    imgGTRoot = os.path.join(opt.realdatapath, clip)
    ...
    for i in range(len(valid_set)):
        ...
        print(fullPaths[i])
        curr_save = os.path.join("results", opt.save, clip)
        curr = os.path.join(curr_save, img_name + '.png')
        
        if os.path.exists(curr):
            print("curr already exists.")
            continue
        ...
        try:
            ...
            print("saving: ", os.path.join(curr_save, img_name + '.png'))
            HQImg = currImg[0:ori_h, 0:ori_w, ...]
            skimage.io.imsave(os.path.join(curr_save, img_name + '.png'), HQImg)
            continue
